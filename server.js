...
// ==================== FIN NOTIFICACIÓN TELEGRAM ====================
  ...
  db = admin.database();
// Limpiar caché de cuotas al iniciar
    ...
    for (const evento of eventosGrupo) {
      for (const game of juegos) {
        ...
        if (
          ...
          if (bookmakers?.markets?.[0]?.outcomes) {
            const outcomes = bookmakers.markets[0].outcomes;
            evento.cuota_local = outcomes.find(o => o.name === 'Away')?.price || evento.cuota_local;
            evento.cuota_visitante = outcomes.find(o => o.name === 'Home')?.price || evento.cuota_visitante;
          }
          break;
        }
      }
