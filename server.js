...
app.get('/api/agents-status', async (req, res) => {
  ...
  // Probar Geminis02 (X-goog-api-key + v1beta + gemini-flash-latest)
  if (geminiKey) {
    try {
      ...
      status.Geminis02 = resp.data?.candidates ? 'online' : 'error: unexpected format';
    } catch(e) { status.Geminis02 = 'error: ' + e.message; }
  } else { status.Geminis02 = 'no_key'; }

  // Probar Agente_groc01 (llama-3.1-8b-instant)
  if (groqKey) {
    try {
      const resp = await axios.post(
      ...
      status.Agente_groc01 = resp.data?.choices ? 'online' : 'error: unexpected format';
